...
This tutorial will get you up and running with the NFTMarketplace contract from the Buidler sample project. When you've done this once, you should have a feel for how to make your Đapps easily.
...
export default func;' > deploy/NFTMarketplace.ts
```
...
- Let's now create a component that consumes a smart contract and gives us some results. From the Buidler sample project, we have a smart contract called `NFTMarketplace.sol`. Open up a third terminal window, create a directory for components, and create a `tsx` file for the view of the smart contract:

`mkdir src/components && touch src/components/NFTMarketplace.tsx`

- Open the file in your favorite editor and copy & paste:

```typescript=
import React, { useContext, useEffect, useState } from "react";
import { NFTMarketplaceContext } from "./../buidler/BuidlerContext";
  ...
  useEffect(() => {
    const doAsync = async () => {
      if (nftMarketplace.instance) {
        console.log("NFTMarketplace is deployed at ", nftMarketplace.instance.address);
        setMessage(await nftMarketplace.instance.greet());
      }
    };
    doAsync();
  }, [nftMarketplace]);
...
Note that the NFTMarketplace context both provides you with
...
import './App.css';
import { BuidlerContext } from "./buidler/BuidlerContext";
import { NFTMarketplace } from './components/NFTMarketplace'; // ADD THIS LINE
    ...
    <div className="App">
      <header className="App-header">
        <BuidlerContext>
          ...
          <NFTMarketplace></NFTMarketplace>  <!-- ADD THIS LINE -->
        </BuidlerContext>
      </header>
    </div>
  );
...
[📚 You can see a full working example of the NFTMarketplace sample here.](https://github.com/symfoni/buidler-react-boilerplate)
...
> Note that since the user will deploy the new tokens, we do not need Buidler to deploy anything. Hence we don't need a `deploy/Token.ts` file, like we did with the nftMarketplace.
